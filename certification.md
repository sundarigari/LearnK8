...
## Create a cheap GCP cluster

Setup Kubernetes on Google Cloud, using g1-small preemptible nodes. Preemptible VMs will run 24h at max and will be removed by Google whenever they want. The cluster, however, supports autoscaling and new capacity will be available within 1 minute. These VMs cost 1/3’rd of the original price, downtime is limited and if availability is really important, you can consider scaling up our Kubernetes pods to get higher availability.
